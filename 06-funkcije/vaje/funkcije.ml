...
let rec mapi f sez =
  let rec mapi_aux i sez' = match sez' with
    | [] -> []
...
let rec unzip l = function
  | [] -> ([], [])
  | (x, y)::l -> 
    let rekurzija = unzip l in
    let xs, ys = rekurzija in (* Druga možnost odpakiranja *)
...
let rec exists f = function
  ...
  | x::xs -> f x || exists f xs (*druga možnost*)
