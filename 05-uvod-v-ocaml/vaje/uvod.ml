...
(* fst da prvi element tupla, snd da drugi element tupla *)
...
let rec repeat_list x n = () (* naredi [x; x; ... x] *)
...
let rec insert x k = function
  | [] -> [x]
  | e::ostalo -> (
    if k <= 0 then x::e::ostalo
    else e:: (insert x (k-1) ostalo)
  )
...
(* Uporabi zgornjo funkcijo divide *))
...
(* Poiščemo največji element, ga damo ven in spet poiščemo največji element *)
