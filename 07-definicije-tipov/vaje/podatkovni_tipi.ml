...
let intbool_separate l = 
       ...
       in separation_pomozna [] [] (intbool_reverse l)
...
let profesor = { name="Matija"; status=Employed (Fire, Teacher) } (* Vrstni red ni pomemben, kot pameten slovar *)
...
(* tri različni načini, vsak bolj len od prejšnjega :D *)
    ...
    match wizard.status with
       | Student (student_magic, years) when student_magic=magic -> (
         if (zahtevana_leta.specialisation) <= years then
           Some wizard.name (* Some ker moramo zapakirati, konstruktor tipa option *) 
  ...
  | wizard::rest -> (
    match wizard.status with
       | Student (student_magic, years) when student_magic=magic && (zahtevana_leta.specialisation) <= years ->
           Some wizard.name (* Some ker moramo zapakirati, konstruktor tipa option, ker je gor napisano None, mora biti tukaj Some, če bi bil zgoraj nek niz, damo lahko samo wizard.name, bolje je uporabiti Some/None *)
       | _ -> find_candidate magic specialisation rest
  )