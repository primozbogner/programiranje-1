...
let optimal_path matrix =
  ...
  let rec optimal_path_aux vrstica_i stolpec_i =
    if vrstica_i > (h-1) || stolpec_i > (w-1) then 0
    else
      let (right, path_right) as r1 = optimal_path_aux vrstica_i (stolpec_i + 1) in
      let (down, path_down) as d1 = optimal_path_aux (vrstica_i + 1) stolpec_i in
      if right >= down then
